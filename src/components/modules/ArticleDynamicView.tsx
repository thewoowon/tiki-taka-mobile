...
const COLORS = {
  ...
  TIKI_DARK_GREEN: theme.colors.tikiDarkGreen,
};
...
export const ArticleDynamicView = () => {
  ...
  const renderArticleItem = useCallback(
    ({ item }: { item: ArticleType }) => <ArticleCard article={item} />,
    [],
  );
    ...
    <View style={styles.headerContainer}>
      <View style={styles.titleRow}>
        <View style={styles.titleContainer}>
          <Text style={styles.title}>취업 준비 꿀팁 아티클</Text>
          <ArticleIcon />
        </View>
      </View>
  ...
  const renderEmpty = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>다른 카테고리를 선택해보세요!</Text>
    </View>
  );
    ...
    if (isFetching && !isFetchingNextPage) {
      return (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>데이터를 가져오고 있어요!🥳</Text>
          <Text style={styles.loadingSubText}>조금만 기다려주세요!</Text>
        </View>
      );
    }
    ...
    <View style={styles.container}>
      ...
      >
        ...
        >
          ...
          >
            <View>
              <Text style={styles.titleText}>기회는 주어지는 것이 아니라,</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={articleList.length === 0 ? renderEmpty : null}
        ListFooterComponent={renderFooter}
        onEndReached={articleList.length > 0 ? handleLoadMore : null}
...
const styles = StyleSheet.create({
  ...
  tagSelected: {
    backgroundColor: COLORS.TIKI_DARK_GREEN,
    borderColor: COLORS.TIKI_DARK_GREEN,
  },
  tagText: {
    fontSize: 16,
    fontWeight: '600',
